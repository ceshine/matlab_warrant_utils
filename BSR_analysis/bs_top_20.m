...
subplot(2,1,1);
bsr_bybroker_chart([cell2mat(result(:,3)), cell2mat(result(:,4))*-1, cell2mat(result(:,5))], result(:,2), 'SELLERS');

query = sprintf(query_template, symbol, from_d, to_d, 'DESC');
result = sql_query(query);
subplot(2,1,2);
bsr_bybroker_chart([cell2mat(result(:,3)), cell2mat(result(:,4))*-1, cell2mat(result(:,5))], result(:,2), 'BUYERS');
