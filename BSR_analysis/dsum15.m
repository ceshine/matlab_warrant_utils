...
query_template = [ 'SELECT date,tbuy, tsell FROM '...
                   ...
                   'JOIN (SELECT SUM(tnet) as tsell,date as ddate FROM bstop15 WHERE tnet < 0 AND company_id = %d GROUP BY date) as bsell ON bbuy.date = bsell.ddate) as a' ];
...
title_s = sprintf('Top 15 ¥D¤O for %d', symbol);
